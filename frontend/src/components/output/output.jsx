...
const FinalPage = () => {
  ...
  const [ebookUrl, setEbookUrl] = useState('./ebook.pdf');
  const [order, setOrder] = useState({
    user: [{ name1: '', email: '' }], // Fallback user structure
    invoiceId: '',
    newDate1: '',
    paymentDetails: { razorpay_order_id: '', razorpay_payment_id: '' }, // Fallback payment details
  });
  ...
  const sendEmail = async (order, fileBlob, filename) => {
    ...
    const userName = order.user[0]?.name1 || 'Guest';  // Fallback if user[0] is undefined
    const userEmail = order.user[0]?.email || 'no-email@example.com';  // Default email
    const razorpayOrderId = order.paymentDetails?.razorpay_order_id || 'N/A';
    const razorpayPaymentId = order.paymentDetails?.razorpay_payment_id || 'N/A';
    ...
    try {
      ...
      alert("Email sent successfully with receipt and eBook!");
    ...
    useEffect(() => {
        ...
        if (order.cart.length > 0) { // Adjust condition, it should be `> 0` if checking non-empty cart
            fetchReceiptUrl();
          }
        }, [order]);
